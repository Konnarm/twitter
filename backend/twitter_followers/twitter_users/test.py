...
class SecondLineFollowersCounterTestCase(TestCase):
    def setUp(self):
        self.second_followers = SecondLineFollowersCounter.objects.create(**test_data_second_line)

    def test_created(self):
        self.assertEqual(SecondLineFollowersCounter.objects.all().count(), 1)

    def test_user_data(self):
        self.assertEqual(self.second_followers.screen_name, test_data_second_line.get('screen_name'))
        self.assertEqual(self.second_followers.user_id, test_data_second_line.get('user_id'))
        self.assertEqual(self.second_followers.followers, test_data_second_line.get('followers'))
...
class FollowersEndpointTestCase(TestCase):
    ...
    def test_endpoint_response_with_data(self):
        response = self.client.get(f'/followers/get_followers/{test_data_second_line.get("screen_name")}/')
        self.assertTrue(isinstance(response.data, dict))
        self.assertEqual(response.data.get('followers'), test_data_second_line.get('followers'))
